## Fase 8 · Apartado 2 — ✅ Qué tienes que entregar

> **[Módulo: SOR — Sistemas Operativos en Red]** · **Integración del Cliente (Windows 11)**
> 🧭 Índice de la fase: [[Fase_8_Integracion_del_Cliente_Windows_11]]
>
> **📍 Cuándo se lee:** **Antes de encender la VM.** Qué tienes que producir.

---

> [!abstract] 📦 Los cuatro entregables
> **Léelo antes de empezar.** Esto es lo que se te va a pedir al terminar:
>
> | Entregable | Dónde vive | Qué debe contener |
> | :--- | :--- | :--- |
> | **Entrada de apuntes** | `00_Apuntes/Trimestre_N/B2_Ubuntu_Local/b2-8-integracion-del-cliente.md` | El procedimiento con tus palabras + **respuestas a las preguntas críticas** + **enlace del vídeo** |
> | **Vídeo** | Playlist `B2_Ubuntu_Local` (No listado) | `B2 · F8 · Integración del cliente`, con presentación, identidad y timestamps |
> | **Repositorio** | Tu repo de apuntes en GitHub | La entrada, subida con `git add` → `commit` → `push` |
> | **💾 Punto de control** | Instantánea en VirtualBox | Nombrada **`Fase 8 terminada`**, tomada **durante la grabación** |

> [!danger] ⚠️ Los nombres NO son orientativos
> El fichero y el vídeo se llaman **exactamente** como pone la tabla. Con un grupo entero entregando, si cada uno pone el nombre que le apetece, corregir se vuelve imposible y **tu entrega se pierde**.

> [!success] 🎯 Criterio de éxito
> Abro tu repositorio, encuentro la entrada de esta fase, y dentro está: qué has hecho, qué has entendido, qué dudas te han quedado y el enlace al vídeo donde se te ve haciéndolo. Si falta el enlace o faltan las respuestas, la fase **no cuenta como entregada**.

---

| ← Anterior | 🧭 Índice | Siguiente → |
| :--- | :---: | ---: |
| [[Fase_8.1_Que_Se_Evalua]] | [[Fase_8_Integracion_del_Cliente_Windows_11]] | [[Fase_8.3_Obligaciones_Grabacion]] |
