...
> 1. **Prepárate primero (sin grabar):** comprueba lo necesario, **léete el procedimiento entero** y **crea la entrada de apuntes de esta sub-fase** en Obsidian: fichero `v1-fase-1-4-verificacion-y-acceso-remoto.md` dentro de `00_Apuntes/Trimestre_N/B2_Ubuntu_Local/`, con la estructura de la Fase 0.1 y **vacía**. Rellenarla es cosa tuya, después.
> 2. **Arranca OBS y PRESÉNTATE:** *"Hola, me llamo [Nombre], 2.º SMR, y en este vídeo voy a explicar la Fase 1.4 de Boochan V1 — Verificación y Acceso Remoto."* Y **muestra algo que demuestre que eres tú** (tu perfil de GitHub, tu Teams o tu correo `@alu.edu.gva.es`). Di qué vas a hacer.
...
> 5. **Al terminar:** nombra el vídeo `V1 · Fase 1.4 — Verificación y Acceso Remoto`, súbelo a tu playlist de YouTube **`B2_Ubuntu_Local`** (No listado) y **copia su enlace**.
...
> > El fichero se llama **exactamente** `v1-fase-1-4-verificacion-y-acceso-remoto.md` y el vídeo **exactamente** `V1 · Fase 1.4 — Verificación y Acceso Remoto`. No es una manía: con cuatro sub-fases por alumno y un grupo entero, si cada uno pone el nombre que le apetece, corregir se vuelve imposible y **tu entrega se pierde**. Un nombre distinto es una entrega no localizada.
